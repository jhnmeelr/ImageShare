...
export function fetchImages(userId = null) {
  ...
  return dispatch => {
    ...
    api.fetchImages(userId)
      .then(images => dispatch({ type: actionNameSuccess, images }))
      .catch(error => dispatch({ type: actionNameError, error }));
  };
}
...
export function addImage(data = null) {
  return dispatch => {
    ...
    api.addImage()
      .then(imageSrc => dispatch({ type: 'ADD_IMAGE_SUCCESS', imageSrc }))
